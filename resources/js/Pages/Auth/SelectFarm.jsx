...
const SelectFarm = ({ farms }) => {
    return (
        <main class="mx-auto flex w-full max-w-2xl flex-grow flex-col items-center justify-start px-4 pb-32 pt-24">
            ...
            <div class="w-full space-y-4">
                {farms.map((farm) => {
                    return <CardFarm key={farm.id} farm={farm} />;
                })}
            </div>

            <div class="fixed bottom-0 left-0 z-40 flex w-full justify-center border-t border-outline-variant/30 bg-surface p-6">
                <button class="flex w-full max-w-md items-center justify-center gap-2 rounded-xl bg-primary py-4 font-bold text-white shadow-lg shadow-primary/20 transition-all hover:bg-secondary active:scale-95">
                    Continuar
