...
const ModalFormTypeGrass = ({ grass = null, onClose }) => {
    const isEdit = !!grass;
    ...
    function handleSubmit(e) {
        ...
        if (isEdit) {
            put(route('type-grasses.update', grass.id), options);
        } else {
            post(route('type-grasses.store'), options);
        }
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label className="mb-1.5 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-widest text-gray-500">
                        ...
                        Nombre del tipo de pasto
                    </label>
                    <input
                        type="text"
                        value={data.name}
                        onChange={(e) => setData('name', e.target.value)}
                        placeholder="Ej. Kikuyo, Brachiaria, Pará..."
                        autoFocus
                ...
                <div className="flex justify-end gap-2 pt-2">
                    ...
                    >
                        ...
                        {isEdit ? 'Guardar cambios' : 'Crear tipo de pasto'}
                    </button>
                </div>
            </form>
        </div>
...
export default ModalFormTypeGrass;
